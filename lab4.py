...
# ------------------Employee-----------------------------
class Employee(Worker, IDBWorker, Prototype):
...
# ---------------------Package-----------------------
class Package(IDBWorker, Prototype):
...
class Department(IDBWorker):
    ...
    def save_to_db(self):
        try:
            ...
            cursor.execute(query, type=self.__type, phone=self.__telephone, address=self.__address, district=district_id,
...
class Admin(Worker, IDBWorker):
    ...
    def load_from_db(cls, *identification):
        try:
            ...
            query = """select login
                         ...
                         , department_department_id
                         from admin
                         where login = :login and emp_password = :password"""
            cursor.execute(query, login=identification[0], password=identification[1])
            list1 = cursor.fetchone()
            connection.close()
            if list1:
    ...
    def save_to_db(self):
        try:
            ...
            cursor = connection.cursor()
            query = """insert into employees(login, password, name,phone, department_id)
